---
title: "Set up Bank of Anthos's Git repo"
weight: 2
...
git add . && git commit -m "GitOps for Bank of Anthos apps" && git push origin main
```
...
At this stage, the `namespaces-required-networkpolicies` `Constraint` should silently (`dryrun`) complain because we haven't yet deployed any `NetworkPolicies` in the `bankofanthos` `Namespace`. There is different ways to see the detail of the violation. Here, we will navigate to the **Object browser** feature of GKE from within the Google Cloud Console. Click on the link displayed by the command below:
```Bash
...
totalViolations: 1
  ...
  - enforcementAction: dryrun
    kind: Namespace
    message: Namespace <bankofanthos> does not have a NetworkPolicy
    name: bankofanthos
```

## Shift-left Policies evaluation

Another way to see the `Constraints` violations is to evaluate as early as possible the `Constraints` against the Kubernetes manifests before they are actually applied in the Kubernetes cluster. When you created the GitHub repository for the Bank of Anthos apps, you used a predefined template containing a GitHub actions workflow running Continuous Integration checks for every commit. See the content of this file by running this command:
```Bash
...
```Plaintext
build   gatekeeper      2022-06-06T00:53:51.7286839Z     [info] v1/Namespace/bankofanthos: Namespace <bankofanthos> does not have a NetworkPolicy violatedConstraint: namespaces-required-networkpolicies
```
{{% notice tip %}}
